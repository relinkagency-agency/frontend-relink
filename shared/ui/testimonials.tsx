...
export function Testimonials({
    ...
    quoteClassName = "text-white/90"
}: TestimonialsProps) {
    return (
        <div className={`w-full pt-12 pb-16 mx-auto ${className}`}>
            <div className="text-center mb-16 px-4">
                <h3 className={`text-5xl md:text-[64px] font-medium tracking-tight mb-6 mt-4 ${titleClassName}`}>Our clients get real results</h3>
                <div className={`flex flex-col sm:flex-row items-center justify-center gap-3 text-[15px] md:text-base ${subtitleClassName}`}>
            ...
            <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 sm:gap-6 md:gap-8 max-w-7xl mx-auto px-4 md:px-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
                {TESTIMONIALS.map((testimonial) => (
                    <div key={testimonial.id} className="flex flex-col gap-4 sm:gap-6 w-[260px] sm:w-[300px] lg:w-[320px] shrink-0 snap-center">
                        ...
                        <p className={`text-base sm:text-lg lg:text-[16px] font-light leading-relaxed font-relink-neue px-1 ${quoteClassName}`}>
                            "{testimonial.quote}"
                        </p>
                    </div>
                ))}
